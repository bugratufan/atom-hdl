...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'atom-hdl:convert-component-to-instance': () => this.convertComponentToInstance(),
  ...
  AnalyzeEntity() {
      ...
      if (editor) {
        ...
        if (entityMatch) {
          ...
          const portRegex = /(\w+)\s+:\s+(in|out)\s+(\w+)\s*(?:\((\d+)\s+(?:downto|to)\s+(\d+)\))?/gmi;
          let portMatch;
          this.instanceTextToPaste = `${entityName}_inst : ${entityName} port map (\n`;
          ...
          while ((portMatch = portRegex.exec(entityMatch[0])) !== null) {
            ...
            } else if (portName.startsWith('o_')) {
              signalName = 's_' + portName.substring(2);
            } else {
              signalName = 's_' + portName;
            }
